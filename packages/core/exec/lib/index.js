...
async function exec() {
  ...
  let storeDir = '';
  let pkg;
  log.verbose('[Core/exec] 检查调试区:', targetPath || '未指定(默认使用线上版本)');
  log.verbose('[Core/exec] 检查缓存路径:', homePath);
  ...
  if (!targetPath) {
    ...
    log.verbose('[Core/exec] No targetPath model.');
  ...
  if (rootFile) {
    try {
      ...
      log.verbose(`[Core/exec] ${cmdName}命令进程:`, child.pid);
      child.on('error', e => {
        log.error(e);
        process.exit(1);
      });
      child.on('exit', e => {
        console.log();
        log.verbose(`[Core/exec] End with code ${e}.\n`);
        process.exit(e);
      });
    } catch (e) {
      log.error(e);
