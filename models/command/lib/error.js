...
const Error_INIT_IS_NOT_EXSITS = () => {
  throw new Error('[Command] init必须实现!');
}

const Error_EXEC_IS_NOT_EXSITS = () => {
  throw new Error('[Command] exec必须实现!');
}

const Error_NODE_VERSION_IS_TOO_LOW = (lowestVersion) => {
  throw new Error(colors.red(`der-cli 需要安装 v${lowestVersion} 以上版本的 Node.js (官网http://nodejs.cn/download/)`));
}
